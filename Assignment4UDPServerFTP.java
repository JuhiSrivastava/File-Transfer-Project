...
class ServerThread1 extends Thread {
	...
	ServerThread1(DatagramSocket outSocket, DatagramSocket inSocket, String dowork, String[] fileNameSource,
			Logger logger) {
	...
	public void run() {
		...
		if (work.equals("send")) {
			...
			for (int i = 0; i < this.srcFiles.length; i++) {
				noOfPacketLost = 0;
				try {
					InetAddress id = InetAddress.getByName("192.168.142.129");
					...
					while (timedOut) {
						try {
							...
							System.out.println("Sending a file to client: " + " " + conn);
							byte[] sendBytes = conn.getBytes();
							DatagramPacket sendDatagramPacket = new DatagramPacket(sendBytes, conn.length(), id, 9876);
							...
							System.out.println("Client: " + acknowledgement);
							timedOut = false;
						} catch (SocketTimeoutException exception) {
							System.out.println("Timeout in receiving the client's status");
							noOfPacketLost++;
					...
					loggerFile.info("Start Time: " + startTime + " - Started to send the file: " + this.srcFiles[i]
							+ " from server to client of size: " + buffer.length);
					int counter = 0;
					...
					while (counter < buffer.length) {
						...
						while (timedOut) {
							try {
								DatagramPacket packet = new DatagramPacket(sendData, sendData.length, id, 9876);
					...
					while (timedOut) {
						try {
							...
							DatagramPacket received = new DatagramPacket(receiveData, receiveData.length);
							sendSocket.receive(received);
							String msgFromClient = new String(received.getData(), 0, received.getLength());
					...
					loggerFile.info("End Time: " + endTime + " - File: " + this.srcFiles[i]
							+ " is fully transmitted from server to client of size: " + buffer.length);
					fis.close();
				...
				try {
					...
					while (checkFilebool) {
						...
						if (msg.equalsIgnoreCase("EOF")) {
							...
							checkFilebool = false;
							moreFiles = false;
						} else if (msg != oldmsg && fileDetails[0].substring(0, 33)
								.equalsIgnoreCase("/home/vmtwo/Desktop/ClientSource/")) {
							checkFilebool = false;
							oldmsg = msg;
						}
					}
					...
					if (!moreFiles || eof) {
						...
						DatagramPacket sendDatagramPacket = new DatagramPacket(sendBytes, sendBytes.length, id, 6789);
					...
					String checkFile = "/home/vmone/Desktop/ServerSource/" + arr[arr.length - 1];
					String conn;
					if (Arrays.asList(this.srcFiles).contains(checkFile)) {
						conn = "Sorry Server is already having this file: " + checkFile;
						System.out.println("Sending to Client: " + conn);
						byte[] sendBytes = conn.getBytes();
						DatagramPacket sendDatagramPacket = new DatagramPacket(sendBytes, conn.length(), id, 6789);
						receiveSocket.send(sendDatagramPacket);
						continue;
					}
					conn = "Server is ready to receive: " + msg;
					System.out.println("Sending to Client: " + conn);
					byte[] sendBytes = conn.getBytes();
					DatagramPacket sendDatagramPacket = new DatagramPacket(sendBytes, conn.length(), id, 6789);
					receiveSocket.send(sendDatagramPacket);
					...
					String newFile = "/home/vmone/Desktop/ServerDestination/" + arr[arr.length - 1];
					...
					loggerFile.info("Start Time: " + startTime + " - Started to receive the file: " + fileDetails[0]
							+ " from client to server of size: " + fileSize);
						...
						while (checkConnection) {
							try {
								...
								if (msg1.equalsIgnoreCase("EOF")) {
									...
									DatagramPacket sendPacket = new DatagramPacket(sending, sending.length, id, 6789);
								...
								if (lastSeqNo == seqNo) {
									...
									DatagramPacket packet = new DatagramPacket(sendData, sendData.length, id, 6789);
					...
					loggerFile.info("End Time: " + endTime + " - File: " + fileDetails[0]
							+ " is successfully received from client to server of size: " + fileSize);
					fos.close();
				} catch (IOException ex) {
					System.out.println(ex);
				}
...
class Assignment4UDPServerFTP {
	public static String[] fileNameSource = { "/home/vmone/Desktop/ServerSource/ServerFile.png",
			"/home/vmone/Desktop/ServerSource/ServerFile.mp4", "/home/vmone/Desktop/ServerSource/ServerFile.pdf",
			"/home/vmone/Desktop/ServerSource/ClientFile.mp4", "/home/vmone/Desktop/ServerSource/ServerFile.txt" };

	public static void main(String args[]) throws IOException {

		Assignment4UDPServerFTP obj = new Assignment4UDPServerFTP();
		...
		DatagramSocket receiveSocket = new DatagramSocket(4321);
		sendSocket.setSoTimeout(2000);
		ServerThread1 sct = new ServerThread1(sendSocket, receiveSocket, "send", setpriority, logger);
		sct.start();
		ServerThread1 rct = new ServerThread1(sendSocket, receiveSocket, "receive", setpriority, logger);
		rct.start();
	...
	public Logger LoggingFiles() {
		...
		try {
			...
			fh = new FileHandler("/home/vmone/Desktop/ServerDestination/ServerLogFile.log");
